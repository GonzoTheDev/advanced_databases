...
ANALYZE SELECT V.EDYEAR,
       ...
       SUM(
           CASE
               WHEN V.EDYEAR BETWEEN 2013 AND 2015 AND V.VOTEMODE IN ('Facebook', 'Instagram') THEN V.VOTE_COST
               WHEN V.EDYEAR BETWEEN 2013 AND 2015 AND V.VOTEMODE IN ('TV', 'Phone') THEN V.VOTE_COST
               WHEN V.EDYEAR BETWEEN 2016 AND 2022 AND V.VOTEMODE IN ('Facebook', 'Instagram') THEN V.VOTE_COST
               WHEN V.EDYEAR BETWEEN 2016 AND 2022 AND V.VOTEMODE IN ('TV', 'Phone') THEN V.VOTE_COST
               ELSE 0
           END * CASE WHEN V.VOTE_CATEGORY = (SELECT CATID FROM VIEWERCATEGORY_DIM WHERE CATNAME = 'Jury') THEN 0 ELSE V.VOTE END
       ) AS TOTAL_INCOME
FROM VOTES_FACT V
JOIN COUNTY_DIM C ON V.COUNTYID = C.COUNTYID
WHERE V.EDYEAR IN (2013, 2019)
